...
class Watershed(object):
   # opject is an array of PNG file
   MASK = -2
   WSHD = 0
   INIT = -1
   INQE = -3
   ...
   # Neighbour (coordinates of) pixels, including the given pixel. 协调/坐标 像素
   def _get_neighbors(self, height, width, pixel): 
      # 获取像素点的neighbor 并返回坐标矩阵
      print("\n\n\n\nget neighbours ", height, width)
      return np.mgrid[ # split mesh
         max(0, pixel[0] - 1):min(height, pixel[0] + 1),
         max(0, pixel[1] - 1):min(width, pixel[1] + 1)
      ].reshape(2, -1).T # reshape and transpose the array
   ...
   def apply(self, image):
      ...
      total = height * width  #用于reshape后的矩阵
      labels = np.full((height, width), self.INIT, np.int32)   #初始化label矩阵

      reshaped_image = image.reshape(total)  # [y, x] pairs of pixel coordinates of the flattened image.
      
      pixels = np.mgrid[0:height, 0:width].reshape(2, -1).T    
      neighbours = np.array([self._get_neighbors(height, width, p) for p in pixels])    # Coordinates of neighbour pixels for each pixel.
...
if __name__ == "__main__":
   ...
   w = Watershed()
   # image = np.array(Image.open(sys.argv[1])) #系统函数调用

   labels = w.apply(image) 
   # cv2.imwrite('ws.png', labels)
